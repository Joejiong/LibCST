...
class Py2Raise(BaseSmallStatement):
    ...
    def _visit_and_replace_children(self, visitor: CSTVisitorT) -> "Raise":
        return Raise(
            ...
            second_comma=visit_sentinel(self, "second_comma", self.second_comma, visitor),
...
class ExecTarget(CSTNode):
    ...
    def _validate(self) -> None:
        ...
        if self.target_locals is None and self.comma is not MaybeSentinel.DEFAULT:
            raise CSTValidationError(
                "There must be a target locals to have a comma."
            )

    def _visit_and_replace_children(self, visitor: CSTVisitorT) -> "ExecTarget":
        return ExecTarget(
            whitespace_before_in=visit_required(
                self, "whitespace_before_in", self.whitespace_before_in,
                visitor),
            whitespace_after_in=visit_required(
                self, "whitespace_after_in", self.whitespace_after_in,
                visitor),
            target_globals=visit_required(self, "target_globals",
                self.target_globals, visitor),
            comma=visit_sentinel(self, "comma", self.comma, visitor),
            target_locals=visit_optional(self, "target_loclas",
                self.target_locals, visitor),
